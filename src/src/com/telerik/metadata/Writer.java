...
public class Writer
{
	...
	{
		int position = (int) outStringsStream.getChannel()
			.position();
	...
	public static void writeTree(String outputDir, TreeNode root) throws Exception
	{
		...
		while (!d.isEmpty())
		{
			...
			{
				n.offsetValue = (int) outValueStream.getChannel()
					.position();
			...
			else if (((n.nodeType & TreeNode.Class) == TreeNode.Class) || ((n.nodeType & TreeNode.Interface) == TreeNode.Interface))
			{
				...
				for (int i = 0; i < len; i++)
				{
					...
					int pos = uniqueStrings.get(fi.name)
						.intValue(); // get start position of the name
					writeInt(pos, outValueStream); // write start position of
													// the name of the variable
					writeTreeNodeId(fi.valueType, outValueStream); // pointer to
																	// the value
																	// type of
																	// the
																	// variable
